...
describe 'DateRanger week mode', ->
  it 'should default range to current week', ->
    dateRanger = TestUtils.renderIntoDocument(<DateRanger period="W" />)
    ...
    expect(fromMoment.format('YYYY WW')).toBe(moment().format('YYYY WW'))
    expect(fromPicker.querySelectorAll('button.highlighted').length).toBe(0)
    expect(toMoment.format('YYYY WW')).toBe(moment().format('YYYY WW'))
  ...
  it 'should preset range if provided', ->
    ...
    dateRanger = TestUtils.renderIntoDocument(<DateRanger range={range} period="W" />)
    [fromPicker, toPicker] = ReactDOM.findDOMNode(dateRanger).querySelectorAll('.react-date-ranger-date-picker')
    fromMoment = moment(fromPicker.querySelector('button.selected').value)
    toMoment = moment(toPicker.querySelector('button.selected').value)

    expect(fromMoment.isSame(moment(range[0]), 'week')).toBeTruthy()
    expect(fromPicker.querySelectorAll('button.highlighted').length).toBe(28)
  ...
  it 'should pick range and fire onChange event', ->
    ...
    TestUtils.Simulate.click(toDays[15])  # 15/02/2016

    expect(range[0].format("YYYY-MM-DD")).toBe("2016-02-08")
    expect(range[1].format("YYYY-MM-DD")).toBe("2016-02-21")
